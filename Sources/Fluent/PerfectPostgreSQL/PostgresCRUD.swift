...
class PostgresCRUDRowReader<K : CodingKey>: KeyedDecodingContainerProtocol {
	...
	func decode<T>(_ type: T.Type, forKey key: Key) throws -> T where T : Decodable {
		...
		case .codable:
            ...
            //yTest
            if type is CRUDEnumCodable.Type {
                return (type as! CRUDEnumCodable.Type).init(data0) as! T
            }
...
class PostgresGenDelegate: SQLGenDelegate {
	...
	func getColumnDefinition(_ column: TableStructure.Column) throws -> String {
		...
		case is Int.Type:
            //yTest
            if column.properties.contains(.serialKey) {
                typeName = "serial"
            } else {
                typeName = "bigint"
            }
...
/// 扩展枚举类型
public protocol CRUDEnumCodable {
